...
class GaussianModel:
    ...
    def load_ply(self, path, shifted=False):
        ...
        if shifted:
            logger.info(f"we shift the point clouds")
            min_xyz = np.min(xyz, axis=0)  # shape: (3,)
            # Step 3: 平移所有点，使最小点对齐原点
            xyz = xyz - min_xyz
        logger.info(f"plydata.elements[0].properties:{plydata.elements[0].properties}")
        elem = plydata.elements[0]
        # 获取所有属性名
        prop_names = [p.name for p in elem.properties]
        labels=None
        if 'label_0' in prop_names:
            logger.info(f"Loading data with labels")
        ...
        if labels is not None:
            ...
            self.mask_hair = self.get_label[..., 0] > 0.5
            self._xyz = self._xyz[self.mask_hair]
            self._features_dc = self._features_dc[self.mask_hair]
            self._features_rest = self._features_rest[self.mask_hair]
            self._opacity = self._opacity[self.mask_hair]
            self._scaling = self._scaling [self.mask_hair]
            self._rotation = self._rotation[self.mask_hair]
