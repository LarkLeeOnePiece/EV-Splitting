...
class CamWidget(Widget):
    def __init__(self, viz):
        ...
        self.cam_pos = torch.tensor([0.0, -6.52, 0.0])
        self.up_vector = torch.tensor([0.0, -1.0, 0.0])
        self.forward = torch.tensor([0.0, 0.0, -1.0])

        # controls
        self.pose = EasyDict(yaw=0, pitch=0)#self.pose.yaw:0.05000000000000002,self.pose.pitch:1.5707963267948966
        self.invert_x = False
        self.invert_y = False
        ...
        # logger.info(f"self.forward:{self.forward},self.cam_pos:{self.cam_pos},self.up_vector:{self.up_vector}")
    ...
    def handle_dragging_in_window(self, x, y, width, height):
        ...
        # logger.info(f"self.pose.yaw:{self.pose.yaw},self.pose.pitch:{self.pose.pitch}")
